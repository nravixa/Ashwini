...
const Navbar = React.memo(function Navbar() {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 mx-auto w-full max-w-[1440px] px-4 sm:px-8 md:px-16 flex justify-center transition-all duration-300 ${scrolled ? 'pt-3' : 'pt-4'}`}
      >
        <div 
          className={`flex justify-between items-center w-full border rounded-full px-6 md:px-10 py-3.5 transition-all duration-300 ${scrolled ? 'bg-[#1d1a31]/85 backdrop-blur-md border-white/10' : 'bg-transparent border-transparent'}`}
        >
          <Link to="/"
            onClick={(e) => handleLinkClick("/", e)}
            className="font-display text-2xl font-bold tracking-tighter text-white uppercase"
          >
            ASHWINI SALON
          </Link>
